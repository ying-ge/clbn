---
title: "clbn mutant vs. wildtype microarray analysis"
author: "Ying Ge"
date: "2018/1/21"
output: html_document
---

email: geying.tju@gmail.com
...
The raw microarray data was deposited under the NCBI GEO database [GSE125032](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE125032).
...
write.csv(exp.eset, file="Output/signal_probeset.csv", quote = F)
```

### Add gene symbol to expression values

```{r}
eset <- read.csv("Output/signal_probeset.csv", row.names = 1)
dim(eset)
...
write.csv(eset_uniq, file="Output/signal_gsym.csv",row.names = T,quote = F)
```
...
```{r}
library(limma)
eset_uniq <- read.csv("signal_gsym.csv", row.names = 1)
...
write.csv(topTable(mulg.fit, coef=1, number = nrow(mulg.fit), adjust="BH"), file="Output/limma_output.csv")
```
...
allgeneslimma <- read.csv("Output/limma_output.csv", row.names = 1)
...
head(eset_DEG)
write.csv(eset_DEG, "Output/DEG_signal.csv", quote = F, row.names = T)
```
...
```{r}
dmgtf <- read.table("Data/dmel-all-r6.25.gtf.gz", sep = "\t")
...
# Table S1
write.csv(gsym.GO.term.up, "Output/gsym.GO.term.up.csv", quote = F, row.names = F)
write.csv(gsym.GO.term.down, "Output/gsym.GO.term.down.csv", quote = F, row.names = F)
```
...
library(msigdbr)
category <- "C5"
category <- msigdbr(species = "Drosophila melanogaster", 
             category = category)
category <- category[, c(1, 8)]

y <- GSEA(gene.expr, TERM2GENE = category, pvalueCutoff =1)
dim(y@result[y@result$pvalue <= 0.01, ])

# Table S2
write.table(y@result[y@result$pvalue <= 0.01, ], "Output/gsea_C5_output.txt" , sep = "\t", quote = F, row.names = F)
```
...
x <- read.table("Output/gsea_C5_output.txt", sep = "\t", header = T, row.names = 1)
...
ggsave("Figure/GSEA_bubbles.pdf", width = 10, height = 12)
```
...
```{r}
library(pheatmap)
selectgenesID <- read.table("Data/mitoch_geneID.txt")

eset_DEG <- read.csv("Output/DEG_signal.csv", row.names = 1)
...
eset_uniq <- read.csv("Output/signal_gsym.csv", row.names = 1)
...
pheatmap <- pheatmap(eset_signaling,cellwidth = 20, cellheight = 12, fontsize = 10,
         ...
         border_color = "NA",
         silent=T, 
         filename = "Figure/heatmap_mitoch_scale.pdf")
```

![](/Figure/heatmap_mitoch_scale.pdf)
...
pvol <- p + geom_text_repel(data = subset(selectgenes, logFC> 0),
  ...
  geom_text_repel(data = subset(selectgenes, logFC < 0),
                          ...
                          show.legend = FALSE)
pvol
ggsave("Figure/valcono.pdf", width = 6, height = 4)
```
