...
To investigate the intrinsic mechanisms of Clbn functions, we systemically analyzed genes expression in *clbn* mutant using a microarray assay. 

To identify the functional differences that reflect the transcriptional difference between the *clbn* mutant and wild-type fly, a gene set enrichment analysis (GSEA) with GO biological process was performed. Top 30 biological processes were shown in bubble plot. 

Multiple differentially expressed genes in *clbn* mutant have been reported or inferred with functions in mitochondria, and were shown in volcano plot and heatmap.
...
The volcano plot code is revised based on FigureYa59volcano from FigureYa (Blogger, WeChat Official Accounts).

We thank Dr. Guangchuang Yu from Southern Medical University and FigureYa for the figure technology support.